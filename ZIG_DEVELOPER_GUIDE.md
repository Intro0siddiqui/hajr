...
- **Libc-Decoupling**: Codebase tests must not link libc. For custom guard page probes, tests must use raw POSIX direct syscall wrappers via `std.posix.system.mprotect` instead of `std.c.mprotect`.
